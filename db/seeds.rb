...
20.times {
    Car.create({make:make.sample, model:model.sample, year:rand(1971..2020), color:color.sample, motor:motor.sample, fuel_type:fuel_type.sample, notes:Faker::Lorem.paragraph})
}

Car.all.each{|c| c.update(driver_name:Driver.all.sample.name, year_driven:(c.year+rand(0..5)))}
