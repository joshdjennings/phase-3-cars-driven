...
class CarsController < ApplicationController
  ...
  post '/cars' do
    car = Car.create({ make: params[:make], model: params[:model], year: params[:year], color: params[:color],
                       category: params[:category], motor: params[:motor], fuel_type: params[:fuel_type], transmission: params[:transmission], drive: params[:drive], year_bought: params[:year_bought], driver_name: params[:driver_name], not_sold: true })
    car.to_json
  end
